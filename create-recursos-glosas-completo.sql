-- ============================================
-- SISTEMA DE RECURSOS DE GLOSAS - COMPLETO
-- ============================================
-- Fluxo: Clínica → Operadora → Auditor → Operadora → Clínica
-- ============================================
...
LEFT JOIN financeiro_guias g ON rg.guia_id = g.id
...
-- Trigger para criar histórico automaticamente ao mudar status
DELIMITER $$
CREATE TRIGGER trg_recursos_glosas_status_change
AFTER UPDATE ON recursos_glosas
FOR EACH ROW
BEGIN
    IF OLD.status_recurso != NEW.status_recurso THEN
        INSERT INTO recursos_glosas_historico (
            ...
            NEW.status_recurso,
            'sistema',
            CONCAT('Status alterado de ', OLD.status_recurso, ' para ', NEW.status_recurso),
            NOW()
        );
    END IF;
END$$
...
-- Trigger para atualizar status da guia quando recurso é deferido
DELIMITER $$
CREATE TRIGGER trg_recursos_glosas_deferido
AFTER UPDATE ON recursos_glosas
FOR EACH ROW
BEGIN
    IF NEW.status_recurso = 'deferido' AND OLD.status_recurso != 'deferido' THEN
        UPDATE financeiro_guias
...
1. CLÍNICA cria recurso de glosa:
   - Status: 'pendente'
   - Anexa documentos
   - Histórico registrado automaticamente
   - Notificação enviada para OPERADORA

2. OPERADORA recebe e analisa:
   - Status: 'em_analise_operadora'
   - Pode APROVAR diretamente → Status: 'deferido'
   - Pode NEGAR diretamente → Status: 'indeferido'
   - Pode SOLICITAR PARECER → Status: 'solicitado_parecer'

3. AUDITOR recebe solicitação (se Operadora solicitou):
   - Status: 'em_analise_auditor'
   - Chat disponível com OPERADORA
   - Emite parecer técnico
   - Status: 'parecer_emitido'

4. OPERADORA recebe parecer:
   - Decide baseado no parecer
   - Status final: 'deferido', 'indeferido' ou 'devolvido_clinica'

5. CLÍNICA recebe resposta final:
   - Visualiza decisão e justificativas
   - Histórico completo disponível

REGRAS DE COMUNICAÇÃO:
- Clínica ↔ Operadora: Via histórico e notificações
- Operadora ↔ Auditor: Via chat exclusivo (tabela recursos_glosas_chat)
- Clínica ↔ Auditor: NÃO PERMITIDO

SEGURANÇA:
- Cada perfil tem acesso apenas às suas informações
- Auditores não veem dados de contato de clínicas
- Sistema registra todas as ações (auditoria completa)
*/
